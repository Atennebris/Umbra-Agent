...
- **Daemon architecture** — runs in the background via PM2. Queue tasks, set timers, and manage sessions efficiently.
...
The check command is extracted from the model's final response (the model is instructed to emit it at the end of the `exec` run). This closes the **write → run → read error → fix** cycle without human intervention.
...
umbra providers set-default    # change the active profile
umbra providers delete         # remove a profile
```
...
## Gateway, Routing, Token Economy & Product Maturity
...
```bash
/cursor blinking   # animated block cursor (default)
/cursor static     # steady cursor
```

Persisted to `~/.umbra/runtime-preferences.json`. Applied immediately via ANSI escape `ESC[?12h` / `ESC[?12l`.

### Default runtime mode

```bash
/mode agent   # require approval for destructive tools (default)
/mode full    # auto-allow all tools — no approval dialogs
```

Persisted to `runtime-preferences.json → defaultMode`. Does not affect permission rules saved via "always".
...
Configured via `/usage` in the TUI and persisted in `runtime-preferences.json → usageDetailMode`:
...
**CLI & TUI**
- [ ] `umbra review` — diff-aware code review mode outputting structured `ReviewFinding[]` (type defined in `src/core/contracts.ts`)
